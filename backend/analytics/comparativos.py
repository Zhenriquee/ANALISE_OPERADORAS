...
def _fmt_numero(valor):
    """Formata inteiro/float para número BR (sem decimais para vidas)"""
    try:
        return f"{int(valor):,}".replace(",", ".")
    except:
        return "0"

# ... (Manter função obter_trimestres_anteriores e calcular_variacoes_operadora iguais) ...
...
def calcular_variacoes_operadora(df, id_operadora, trimestre_atual):
    ...
    if trimestre_atual not in df_op.index: return None
...
# ... (Manter calcular_kpis_financeiros_avancados igual) ...
def calcular_kpis_financeiros_avancados(df_mestre, id_operadora, trimestre_atual):
    ...
    if df_hist_window.empty or df_hist_window.iloc[-1]['ID_TRIMESTRE'] != trimestre_atual: return None
    ...
    marca_op = extrair_marca(razao_social, id_operadora)
    df_tri_mercado['Marca_Temp'] = df_tri_mercado.apply(lambda x: extrair_marca(x['razao_social'], x['ID_OPERADORA']), axis=1)
    total_receita_grupo = df_tri_mercado[df_tri_mercado['Marca_Temp'] == marca_op]['VL_SALDO_FINAL'].sum()
...
def calcular_kpis_vidas_avancados(df_mestre, id_operadora, trimestre_atual):
    ...
    # Dados do Trimestre (Mercado Total)
    df_tri_mercado = df_mestre[df_mestre['ID_TRIMESTRE'] == trimestre_atual].copy()
    ...
    return {
        ...
        'Ctx_Share_Nacional': ctx_share_br,
        'Share_Grupo': share_grupo,
        'Ctx_Share_Grupo': ctx_share_grupo,
        'Marca_Grupo': marca_op,
        'CAGR_1Ano': cagr,
